...
- **Chord library** — 10 essential open chords. Tap one to see the shape, hear it strummed, and watch the strings light up in strum order. **🎤 Check my strum** listens to your guitar and tells you whether your strum actually sounds like the chord (chroma matching against the whole library — "hearing something closer to Em" when you're off).
- **Beginner exercises** — spider crawl warm-up, C major scale, A minor pentatonic, and a G–C–D chord-change drill. Each has play/pause, step-through, tempo control, and a lit-string + audible pluck for every note. **🎤 Listen mode** flips the direction: the app hears what you pluck, tells you what note it heard, and auto-advances when you play the right one.
...
- **Fingerpicking tab** — its own library of fingerstyle songs (House of the Rising Sun, Greensleeves, Scarborough Fair, an arpeggio study). The pattern row shows right-hand fingers (p·i·m·a) and target strings; exactly one string lights per pluck, with the thumb automatically following each chord's bass string. AI requests and Claude imports work here too.
